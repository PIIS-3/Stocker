...
#   - created_at → PostgreSQL pone NOW() al hacer el INSERT.
#   - updated_at → PostgreSQL pone NOW() al INSERT y el trigger
...
#       Le dice a PostgreSQL que use NOW() como valor por defecto en INSERT
#       cuando el campo no se envía explícitamente. sa.text() es necesario
#       para que SQLAlchemy trate "NOW()" como SQL crudo y no como un
#       valor Python — lo pasa tal cual a PostgreSQL sin escaparlo.
#   nullable=True
...
class TimestampMixin(SQLModel):
    created_at: Optional[datetime] = Field(
        default=None,
        sa_column_kwargs={"server_default": sa.text("NOW()"), "nullable": True},
    )
